...
document.getElementById('save').onclick=async()=>{status.textContent='Validating…';const apps=[...document.querySelectorAll('.apps input:checked')].map(x=>x.value);const body={provider:document.getElementById('provider').value,api_key:document.getElementById('key').value,apps,mode:'api'};const r=await fetch('/api/configure',{method:'POST',headers,body:JSON.stringify(body)});const d=await r.json();status.textContent=r.ok?`Configured ${d.apps.length} app(s); ${d.models} provider models available.`:(d.detail||'Setup failed')};
</script></body></html>"""
